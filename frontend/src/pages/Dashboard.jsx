...
const Dashboard = () => {
  ...
  useEffect(() => {
    apiService.getVideosForChannel("", "").then((vids) => {
    ...
    <main className="flex p-4 overflow-x-auto">
      <div className="p-4">
        <div className='flex'>
          ...
          <button className='w-full m-1 h-10 mb-4 bg-gray-300 rounded-lg' onClick={() => navigator(`/channel/${channel_id}/editors`)}>All Editors</button>
        </div>
        {open ? <AddEditor handleOpen={handleOpen} /> : <></>}
        <div className="mb-3 w-full max-w-sm bg-gray-900 text-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300" onClick={() => navigator(`/upload`)}>
